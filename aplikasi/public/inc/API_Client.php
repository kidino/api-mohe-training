...
class API_Client {
	var $base_url = 'http://api.mohe.gov.test/';
	var $token = null;
	var $user_agent = 'PHP API Client';
	var $username = null;
	var $password = null;
	
	function __construct($base_url = null, $token = null, $user_agent = null, $username = null, $password = null) {
		if ($base_url != null) {
		...
		if ($back_data != null) {
			...
			if(isset($back_data['message']) && 
				...
				if ($this->login($this->username, $this->password)) {
					return $this->go($uri, $method, $data);
				}
			}
